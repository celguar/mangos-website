 ...
 * them online in various formats at <a href="../www.gnu.org/default.htm">http:../www.gnu.org/default.htm</a> and 
 * <a href="../www.apache.org/default.htm">http:../www.apache.org/default.htm</a>.
 *
        ...
        if(ownerDoc.parseError && ownerDoc.parseError != 0) {
            ...
            if(oTargetElement.tagName.toLowerCase() == "textarea" || oTargetElement.tagName.toLowerCase() == "input") {
                ...
                if(oNode.nodeType == Node.DOCUMENT_NODE || oNode.ownerDocument.documentElement == oNode) {
					...
					var tempString = new XMLSerializer().serializeToString(oNode);

                    oTargetElement.innerHTML = tempString;

					// MFS NOTE: this code (written by Mike Hein) gets <script> tags to run after the xml is transformed
					var tempArray=tempString.split('<script type="text/javascript">')
					for(count=1;count<tempArray.length;count++){
						tempArray2=tempArray[count].split('</script>');
						theScript=tempArray2[0];
						head = document.getElementsByTagName("head");
						script = document.createElement("script");
						head[0].appendChild(script);
						script.type = "text/javascript";
						script.text = theScript;
					}
                }
                else{
