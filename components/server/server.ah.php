...
$query = "SELECT ah.item_template AS item_entry, IF(ah.buyoutprice>0,CAST(ah.buyoutprice AS UNSIGNED),'---') AS buyout, ah.time, IF(ah.buyguid>0,buy.name,'---') AS buyer, IF(ah.lastbid>0, ah.lastbid, ah.startbid) AS currentbid, itm.name AS itemname, itm.class AS `class`, itm.quality, sell.name AS seller, iins.count AS quantity";
$query_part2 = " from `auction` AS `ah` LEFT JOIN ".$mangos['db_name'].".item_template AS itm ON (ah.item_template = itm.entry) LEFT JOIN `characters` as `sell` ON (ah.itemowner = sell.guid) LEFT JOIN `item_instance` AS `iins` ON (ah.itemguid = iins.guid) LEFT JOIN `characters` AS `buy` ON (ah.buyguid = buy.guid)".$ah_where;
